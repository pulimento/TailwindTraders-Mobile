...
if [ -e "$APP_DEFAULT_SETTINGS_FILE" ]
then
    echo "Updating Android AppCenter secret"
    sed -i 's/$ANDROID_APPCENTER_SECRET_PLACEHOLDER/$ANDROID_APPCENTER_SECRET/g' $APP_DEFAULT_SETTINGS_FILE
	
	echo "Updating iOS AppCenter secret"
    sed -i 's/$IOS_APPCENTER_SECRET_PLACEHOLDER/$IOS_APPCENTER_SECRET/g' $APP_DEFAULT_SETTINGS_FILE
	
	echo "Updating Root API URL"
    sed -i 's/$ROOT_API_URL_PLACEHOLDER/$ROOT_API_URL/g' $APP_DEFAULT_SETTINGS_FILE

    echo "Resulting file content:"
    cat $APP_DEFAULT_SETTINGS_FILE
else
    echo "Error! $APP_DEFAULT_SETTINGS_FILE is not found"
	exit
fi