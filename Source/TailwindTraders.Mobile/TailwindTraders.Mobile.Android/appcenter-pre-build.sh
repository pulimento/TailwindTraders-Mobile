...
if [ -e "$APP_DEFAULT_SETTINGS_FILE" ] ; then
	...
	sed -i '.bak' -e 's/__ENTER_YOUR_ANDROID_APPCENTER_SECRET_HERE__/$ANDROID_APPCENTER_SECRET/g' $APP_DEFAULT_SETTINGS_FILE
	echo "Updating iOS AppCenter secret"
	sed -i '.bak' -e 's/__ENTER_YOUR_IOS_APPCENTER_SECRET_HERE__/$IOS_APPCENTER_SECRET/g' $APP_DEFAULT_SETTINGS_FILE
	echo "Updating Root API URL"
	sed -i '.bak' -e 's/__ENTER_YOUR_HTTPS_ROOT_API_URL_HERE__/$ROOT_API_URL/g' $APP_DEFAULT_SETTINGS_FILE
